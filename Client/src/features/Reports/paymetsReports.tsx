...
const PaymentsReport: React.FC = () => {
  ...
  const { data, isLoading, error } = useFetchUserWithPaymentsQuery(
    Number(userId)
  );
  ...
  if (isLoading)
    return <p className="text-center text-lg">Loading payments...</p>;
  if (error || !user)
    return (
      <p className="text-center text-red-500">Failed to load user payments.</p>
    );

  return (
    <div className="max-w-8xl mx-auto p-6 ">
      <div className="flex justify-center items-center w-full">
        <h1 className="text-3xl font-bold mb-6 text-black ">Payment History</h1>
      </div>

      <div className="bg-gray-100 shadow-md p-6 rounded-lg text-black">
        <div className="flex flex-col justify-center items-center ">
          <div>
            ...
            <p className="text-gray-400">
              <strong>Phone:</strong> {user.contact_phone || "N/A"}
            </p>
            <p className="text-gray-400">
          ...
          <div className="grid grid-cols-2 gap-8 mt-2 w-full ">
            {user.payments.map((payment: any) => (
              ...
              >
                ...
                <p>
                  <strong>Property Name:</strong> {payment.property_name}
                </p>
                <p>
                  <strong>Property Type:</strong> {payment.property_type}
                </p>
                <p>
                ...
                <p>
                  <strong>Transaction Date:</strong> {payment.transaction_date}
                </p>
              </div>
            ))}
          </div>
